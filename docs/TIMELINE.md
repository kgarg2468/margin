...
- [x] **C3 — Scout run.** Extract citation gates from synthesis into `lib/citations/` (synthesis refactored to consume it), scheduled-reader gather + guard test, model call (JSON-serialized untrusted context, strict schema, no-retry-of-model-call), sanitizer, failure states.
