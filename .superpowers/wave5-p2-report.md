...
Two independent reviews (correctness, security) returned FIX-FIRST on PR #86 with eleven
findings. All eleven are addressed. Four commits, pushed to `kgarg2468/wave5-p2`:

| commit | covers |
|---|---|
| `ae43034` | F1, F2, F3, F4 — the signature fails closed, and consent outranks the marker |
| `dcf90cb` | F7, F8 — leaving withdraws consent, and anyone can close a paper link |
| `545519f` | F5, F6, F9, F10, F11 — a throttle that counts links, and two guards that now bite |
| `266fea6` | F9 second pass — the throttle spreads out |

## Per-finding disposition

**F1 (CRITICAL) — absent citation snapshot treated as citing nothing.** Fixed. `approvedWriteUp`
now returns `null` when `session.synthesisCitedAnnotationIds === undefined`, so a legacy record
deads the share instead of publishing with the withdrawal check skipped. A test carries the
legacy shape explicitly.

**F2 — reply-threading consent leak.** Fixed as ruled, with the split the ruling asked for. A
root failing for MISSING CONSENT is dropped whole — no card, no marker, no id, no page, no
timestamp — and its consented replies are promoted to standalone cards. A root failing for
WITHDRAWAL keeps the redaction-marker card (the all-or-nothing rule) with `createdAt` taken
from the first surviving reply, or zero when there is none. `redactWhenAnyWithdrawn` is applied
only to consented roots, so the withdrawal path can never be reached by a non-consenting one.
Tests run in both directions. The module's rule comment and `share-panel.tsx:126` copy both
updated to describe the implemented rule.

**F3 — `truncated` counted pre-consent; `PartialNotice` miswired.** Fixed. `truncated` is gone
from `publicPaperView` entirely rather than recomputed — it counted rows fetched, not rows
shown. `PartialNotice` renders unconditionally.

**F4 — mint gated on `session.synthesis`, read gated on `synthesisApprovedAt`.** Fixed. All
three sites (`shareSynthesis`, `forSession`, `approvedWriteUp`) gate on `synthesisApprovedAt`.

**F5 — credential guard missed three real names.** Fixed. `CREDENTIAL_ISH` widened and names
normalized before matching, so the pattern is case-insensitive across camelCase and
SCREAMING_SNAKE. `convexDeployKey`, `openaiKey` and `OPENAI_API_KEY` are each asserted to trip
it. The exemption set is three identifier names (`zoteroItemKey`, `collectionKey`, `key`),
named individually.

**F6 — membership-gate assertion only checked validator paths.** Fixed. The test now reads
`convex/shares.ts` from disk and asserts each of the six management functions contains a
`requireMembership(ctx,` call, and that `view` contains none. Gutting the gate fails the suite.

**F7 — departing member kept `paperShareOptIns`.** Fixed. `endMembership` sweeps the departing
user's opt-in rows for that lab via a new `by_user_and_lab` index. Not ledgered per paper — the
departure is the fact and it is already recorded.

**F8 — asymmetric revoke.** Fixed. `mayRevoke` returns true for any current lab member on a
paper share; synthesis shares stay creator-or-PI; minting unchanged. The copyright policy
question (should the stored publisher PDF ride along on public shares at all) is in the PR body
as a founder decision. No `includePdf` flag added.

**F9 — no throttle on the unauthenticated surface.** Fixed, and it took two passes.

A coarse fixed-window counter keyed by **share id only** — never IP, never user. The reasoning
is in the comment: an aggregate count with a rolling window records nothing about who read, so
it is an abuse guard and not read-tracking. Keyed by share id rather than the token string so
the capability is not copied into a second table where revocation does not reach. Ceiling is
600/minute, generous by design. Convex queries cannot write, so `shares.view` became a
mutation; throttling only the HTTP route would have been defeatable because a public query is
callable straight from the bundle. The throttled page returns `{ busy: true }` — a distinct
shape from `null`, so a good link is never reported dead — and renders a plain busy frame. The
PDF route answers 429.

Then the live load test failed: **640 requests → 25 × HTTP 500**, `OptimisticConcurrencyControl
Failure` on the single counter document. Every reader was writing the same row and Convex
refused the collisions — the guard meant to let a viral link survive was the thing breaking it,
which is precisely what the ruling forbade. Fixed by sharding the counter eight ways
(`by_share_and_shard`, each shard holding an eighth of the ceiling, shard chosen at random) and
adding a single retry in the page's `read()` wrapper before it degrades to busy. Re-verified
live: **700/700 = 200**, no 500s, throttle still engages. `revoke` deletes all shard rows.

The throttle test uses a loop-until-busy `hammer()` rather than a fixed request count, because
random shard selection makes any fixed count flaky.

**F10 — `/shared-pdf` reused authed CORS advertising `Authorization`.** Fixed. Its own
`SHARED_CORS_HEADERS` block with no `Authorization`, used by both OPTIONS and GET. The authed
`/pdf` route is untouched.

**F11 — "nothing about this request is recorded" was false.** Fixed. The comment now admits the
token travels in the query string and therefore reaches Convex's HTTP request log, and that
revocation does not reach log storage. The narrowed claim is that nothing is recorded *per
reader*.

## Verification

All five gates clean on the final tree: `npm test` **1633 passed / 83 files**, `npx tsc
--noEmit`, `npm run typecheck:convex`, `npx eslint .`, `npx next build`. Test count moved
1618 → 1633.

Live re-verification on dev, logged out: load test 700/700 = 200 with the busy page engaging at
the ceiling; then revoked from the panel — page 404, PDF 404, never-minted token 404, all three
identical.

The temporary `labs.personalFor` widening was needed again for this round's dev push and was
reverted again. The committed schema does not contain it.

## Properties held intact

Both reviewers verified these sound and the fixes preserve them: private notes structurally
unread; revoked byte-identical to nonexistent; withdrawal parity stricter than the authed
surface; no XSS path; middleware scope unchanged; the credential guard's four allowances are
exact strings.

## Flagged, not fixed

- The copyright question under F8 — founder's call, noted in the PR body.
- Everything in the original flagged list above still stands.
