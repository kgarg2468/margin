 ...
 *
 * Results are grouped and never mixed, because the three kinds are not
 * comparable — a paper is a place to go, a note is something a colleague said,
 * a session is a date. Ranking them against one another would mean inventing a
 * number; grouping them lets the reader apply the one they already have.
 *
...
/** Every result, in one list, in the order the eye moves down them. */
type Hit =
  | { kind: "paper"; key: string; href: string }
  | { kind: "annotation"; key: string; href: string }
  | { kind: "session"; key: string; href: string };

function flatten(results: SearchResults | null): Hit[] {
  if (results === null) {
    return [];
  }
  return [
...
export function CommandPaletteProvider({ children }: { children: ReactNode }) {
  ...
  // not the palette is on screen — it is what puts it there.
  useEffect(() => {
...
function Palette({ close }: { close: () => void }) {
  ...
  /** Where a group's rows start in the flat list, so a row knows its own index. */
  const paperOffset = 0;
          ...
          {currentLab === null ? (
            ...
            <Aside>
              ...
              <span className="mt-2 block font-sans text-xs text-ink-faint">
                ...
                written, and sessions someone gave a name — not the text inside
                the PDFs.
              </span>
            </Aside>
          ) : (
...
function Row({
  ...
  return (
    <div
      ...
      onClick={onPick}
      className={
        "flex cursor-pointer flex-col gap-0.5 border-l-2 px-5 py-2.5 transition-colors " +
        (active
...
export function SearchAffordance() {
  ...
  return (
    <button
      type="button"
      onClick={open}
      className="flex items-center gap-2 rounded-sm border border-rule bg-surface px-3 py-2 text-left transition-colors hover:border-ink-faint motion-safe:transition-[color,background-color,border-color,transform] motion-safe:duration-200 motion-safe:active:scale-[0.98]"
    >
      <SearchGlyph className="size-3.5 shrink-0 text-ink-faint" />
